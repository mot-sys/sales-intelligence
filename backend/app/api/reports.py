...
# ─────────────────────────────────────────────
# P1.7  CHANGELOG — change-since-last-week
# ─────────────────────────────────────────────
...
async def get_changelog(
    ...
    """
    P1.7: Compare current KPI snapshot vs the previous week's snapshot.
    Returns a structured delta per metric so the frontend can show
    "What changed this week?" with ▲ / ▼ indicators.

    Requires at least two KPI snapshots to exist (generated by the Celery task
    every Monday at 06:00 UTC via P0.7).
    """
    snapshots = await db_crud.get_kpi_snapshots(db, customer_id, limit=2)

    if len(snapshots) < 1:
        return {
            "available": False,
            "reason": "No KPI snapshots available yet. Snapshots are taken every Monday automatically.",
            "current": None,
            "previous": None,
            "deltas": {},
        }
    ...
    previous_metrics = (previous.metrics or {}) if previous else {}

    # Compute deltas for all numeric metrics
    deltas: Dict = {}
    for key, cur_val in current_metrics.items():
        ...
        deltas[key] = {
            ...
            "direction": (
                "up" if (delta or 0) > 0 else
                "down" if (delta or 0) < 0 else
                "flat"
            ),
        }

    # Highlight the most notable changes
    significant = [
        k for k, v in deltas.items()
        if v["pct_change"] is not None and abs(v["pct_change"]) >= 10
    ]

    return {
        "available": True,
        "current_week": str(current.snapshot_date),
        "previous_week": str(previous.snapshot_date) if previous else None,
        "deltas": deltas,
        "significant_changes": significant,
        "summary": _build_changelog_summary(deltas),
    }


def _build_changelog_summary(deltas: Dict) -> List[str]:
    """Build a list of human-readable change bullets."""
    bullets = []
    label_map = {
        "pipeline_value": "Pipeline value",
        "open_deals": "Open deals",
        "closed_won_value": "Closed-won revenue",
        "closed_won_count": "Deals closed",
